...
from self_sup_seg.utils.logger import parse_arguments, logger_level

# import parameters
from self_sup_seg.config.params_data import SEGMENTS_INFO_PATH, ANNOTATION_DIR, SEGMENTS_VAL_SIZE, \
    SEGMENTS_TEST_SIZE, SEGMENTS_DST, RANDOM_SEED, IMAGE_PATH
from self_sup_seg.config.params_meta import _CATEGORIES, _UNKNOWN_ID
...
def create_panoptic_label(annotation, segments):
    ...
    for i, segment in enumerate(segments):
        ...
        meta_segments_id = category_id * _PANOPTIC_LABEL_DIVISOR + instance_count[category_id]
...
def save_meta(segments_info: dict,
              meta_images: list,
    ...
    meta = {
        ...
        'images': [meta_image for meta_image in meta_images if meta_image['id'] in idx_list],
        'annotations': [meta_annotation for meta_annotation in meta_annotations if
                        meta_annotation['image_id'] in idx_list],
...
def main():
    ...
    assert os.path.isfile(SEGMENTS_INFO_PATH), f'No file found under {SEGMENTS_INFO_PATH}'
    ...
    if SEGMENTS_VAL_SIZE > 0:
        ...
        train_idx, val_idx = train_test_split(train_idx, test_size=SEGMENTS_VAL_SIZE, random_state=RANDOM_SEED)
    ...
    if SEGMENTS_TEST_SIZE > 0:
        ...
        train_idx, test_idx = train_test_split(train_idx, test_size=SEGMENTS_TEST_SIZE, random_state=RANDOM_SEED)
    ...
    for idx, ann_info in enumerate(tqdm(annotations_info, desc='Images Converted')):
        ...
        panoptic_label_color, meta_segments = create_panoptic_label(annotation, segments)
        ...
        os.rename(os.path.join(IMAGE_PATH, image_file[:-4] + '.jpg'),
                  os.path.join(path_image, image_file[:-4] + '.jpg'))
        cv2.imwrite(os.path.join(path_anno, image_file), panoptic_label_color[..., ::-1])

    save_meta(segments_info, meta_images, meta_annotations, train_idx, dataset_name='train')
    if SEGMENTS_VAL_SIZE > 0:
        save_meta(segments_info, meta_images, meta_annotations, val_idx, dataset_name='val')
    if SEGMENTS_TEST_SIZE > 0:
        save_meta(segments_info, meta_images, meta_annotations, test_idx, dataset_name='test')
